...
>
> **M19 (2026-04-27): TimeSeriesMemory and AgentMemory also support `nativePackage: 'router'`** with the same publish-ready story. TSM is fully usable on router (naturally append-only). AgentMemory is mostly usable: `forget(id)` throws `CAPABILITY_DEFERRED` (Issue #11) but `remember/recall/recordFeedback` and the SONA loop all run unchanged. Three demos run without `RUVECTOR_CORE_BINDING`: `examples/knowledge-base-router-demo/`, `examples/time-series-memory-router-demo/`, `examples/agent-memory-router-demo/`.
>
> **M20 (2026-04-27): the canonical v0.3 publish-ready demo lives at [`examples/v03-publish-ready-demo/run.mjs`](examples/v03-publish-ready-demo/run.mjs).** All 5 archetypes (LocalLLM + GraphReasoner + KnowledgeBase + TimeSeriesMemory + AgentMemory) wired together in a single process, with the 3 KB-family archetypes on `nativePackage: 'router'`, **mixing dimensions** (KB at 768, TSM at 8, AgentMemory at 768). Validates the M18/M19 multi-instance-isolation property end-to-end: `npm install @ruvector/sdk @ruvector/router @ruvector/graph-node @ruvector/ruvllm @ruvector/sona` and run `node examples/v03-publish-ready-demo/run.mjs`. No env var. No bindingPath.
