...
| [`docs/plans/m11-scope.md`](docs/plans/m11-scope.md), …, [`m17-scope.md`](docs/plans/m17-scope.md), [`m31-scope.md`](docs/plans/m31-scope.md) | Per-milestone scoping passes (LocalLLM / Phase 2 / AgentMemory / AgentFramework / CLI / Transports / AgentMemory full-state persistence). Each ends with ratified open questions whose answers shaped the implementation milestones. |
| [`docs/upstream-issues/`](docs/upstream-issues/) | Paste-ready bug reports for upstream. 8 issues filed as of M17-ratification. |
| [`tools/reprobe-bindings/reprobe.mjs`](tools/reprobe-bindings/reprobe.mjs) | Ground-truth on upstream surface contracts. Re-runnable; CI-gateable. v0.5 tracks 31 npm + 1 CLI + 6 binding-method probes (M22+M23: subprocess-isolated, SIGKILL-on-timeout for hung NAPI calls per Issue #11; `'skipped'` status via exit code 64 for prerequisite-unavailable cases). Covers Issues #01/#03/#06/#09/#10/#11. **Coverage map + result-type semantics + cadence** are formalized in [PRD §11.6](docs/plans/ruvector-sdk-prd.md). |
